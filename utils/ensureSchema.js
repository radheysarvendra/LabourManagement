...
const ensureSchema = async (db) => {
  ...
  // Individual identity verification. This reuses the legacy verifications
  // table but removes any requirement for a labour/owner/contractor profile.
  await ensureTable(db.sequelize, "verifications", `
  ...
  `);
  await ensureColumn(queryInterface, "verifications", "user_id", { type: db.Sequelize.INTEGER, allowNull: true });
  await ensureColumn(queryInterface, "verifications", "photo_url", { type: db.Sequelize.TEXT, allowNull: true });
  await ensureColumn(queryInterface, "verifications", "photo_cloudinary_public_id", { type: db.Sequelize.TEXT, allowNull: true });
  await ensureColumn(queryInterface, "verifications", "submitted_at", { type: db.Sequelize.DATE, allowNull: true });
  await ensureColumn(queryInterface, "verifications", "verified_by_admin_id", { type: db.Sequelize.INTEGER, allowNull: true });
  await ensureColumn(queryInterface, "verifications", "is_locked", { type: db.Sequelize.BOOLEAN, allowNull: false, defaultValue: false });
  await db.sequelize.query(`ALTER TABLE "verifications" ALTER COLUMN "labour_id" DROP NOT NULL`).catch(() => {});
  await db.sequelize.query(`ALTER TABLE "verifications" ALTER COLUMN "aadhar_number" DROP NOT NULL`).catch(() => {});
  await db.sequelize.query(`ALTER TABLE "verifications" ALTER COLUMN "status" SET DEFAULT 'NOT_UPLOADED'`).catch(() => {});
  await db.sequelize.query(`ALTER TABLE "verifications" DROP CONSTRAINT IF EXISTS "verifications_status_check"`);
  await db.sequelize.query(`
  ...
  await db.sequelize.query(`
    ...
    CHECK ("status" IN ('NOT_UPLOADED', 'PENDING_REVIEW', 'VERIFIED', 'REJECTED'))
  `);
  await db.sequelize.query(`ALTER TABLE "verifications" ALTER COLUMN "id" ADD GENERATED BY DEFAULT AS IDENTITY`).catch(() => {});
  await db.sequelize.query(`
    UPDATE "verifications" v
    SET "user_id" = l."userId"
    FROM "labours" l
    WHERE v."user_id" IS NULL
      AND (v."labour_id" = l.id::text OR v."labour_id" = l."labourCode")
  `).catch(() => {});
  await db.sequelize.query(`
    DO $$ BEGIN
      IF NOT EXISTS (SELECT 1 FROM pg_constraint WHERE conname = 'verifications_user_id_fkey') THEN
        ALTER TABLE "verifications"
    ...
    END $$
  `);
  await db.sequelize.query(`CREATE UNIQUE INDEX IF NOT EXISTS "idx_verifications_user_id" ON "verifications" ("user_id") WHERE "user_id" IS NOT NULL`);
